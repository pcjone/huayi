...
import com.huayi.common.util.DataUtil;
import com.huayi.common.util.DateUtil;
import com.huayi.common.util.InstanceUtil;
import com.huayi.common.util.StringUtil;
 ...
 */
public abstract class BaseServiceImpl<T extends BaseModel> extends BaseLogger{
	...
	public void deleteDBAndCache(Long[] ids) {
		try {
			...
			for (Long id : ids) {
				list.add(id);
				CacheUtil.getRedisCacheHelper().del(getCacheKey(id));
			}
	...
	public void cancelDBAndCache(Long[] ids, String account) {
		try {
			for (Long id : ids) {
				...
				CacheUtil.getRedisCacheHelper().set(getCacheKey(id), record);
			}
		} catch (Exception e) {
			throw new RuntimeException(e.getMessage(), e);
		}
	...
	public T updateDBAndCache(T record) {
		try {
			...
			CacheUtil.getRedisCacheHelper().del(getCacheKey(record.getId()));
		} catch (Exception e) {
			throw new RuntimeException(e.getMessage(), e);
		}
		return record;
	...
	public T queryCacheById(Long id) {
		try {
			...
			T record = (T) CacheUtil.getRedisCacheHelper().get(key);
			if (record == null) {
				record = getMapper().selectByPrimaryKey(id);
				CacheUtil.getRedisCacheHelper().set(key, record);
			}
			return record;
		} catch (Exception e) {
			throw new RuntimeException(e.getMessage(), e);
