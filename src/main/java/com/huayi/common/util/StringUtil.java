...
public class StringUtil {
	...
	public static String getOrderCondition(String sort, String order) {
		String orderFieldStr = "";
		if (sort != null && sort.length() != 0) {
			//骆驼式命名转换为下滑线命名，数据库设计是以"_"命名规则，否则直接用orderField
			orderFieldStr = CamelUtil.camelToUnderline(sort);//orderField
			orderFieldStr = orderFieldStr + " " + order;
		}
		return orderFieldStr;
	}

	public static void main(String[] args) {
		System.out.println(getOrderCondition("categoryName", "desc"));
	}
