package com.huayi.common.cache;
...
import com.huayi.common.config.PropertyConfigurer;
import com.huayi.common.util.InstanceUtil;

public final class RedisCacheHelper implements CacheManager, ApplicationContextAware{
	
	private RedisTemplate<Serializable, Serializable> redisTemplate = null;
    private Integer EXPIRE = PropertyConfigurer.getInt("redis.expiration");
    ...
    private RedisTemplate<Serializable, Serializable> getRedis() {
        if (redisTemplate == null) {
            synchronized (RedisCacheHelper.class) {
                if (redisTemplate == null) {
	 ...
	 * 设置缓存  缓存没有失效时间
	 */
	public final void setNoExpiry(final String key, final Serializable value) {
	 ...
	 * 查询缓存  缓存没有失效时间
	 */
	public final Serializable getNoExpiry(final String key) {
