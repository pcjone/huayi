...
public class WxArticleController extends BaseController{
	private final Logger logger = Logger.getLogger(WxArticleController.class);
	@Autowired
	...
	* @throws
	 */
	@RequiresPermissions("wei.wxArticle.list")
	...
	* @throws
	 */
	@RequiresPermissions("wei.wxArticle.dataList")
	@RequestMapping(value="/dataList",method = RequestMethod.POST)
	public void dataList(HttpServletRequest request, HttpServletResponse response) {
		Map<String, Object> params = WebUtil.getParameterMap(request);
		PageInfo<WxArticle> pageList = wxArticleService.query(params);
	...
	* @throws
	...
	public void queryById(Long id,HttpServletRequest request, HttpServletResponse response) {
		WxArticle record = wxArticleService.queryCacheById(id);
	...
	* @throws
	...
	public void save(HttpServletRequest request, HttpServletResponse response) {
		WxArticle record = Request2ModelUtil.covert(WxArticle.class,request);
		if(record != null) {
			if(record.getId() == null) {
				record.setCreateBy(getCurrUser());
				wxArticleService.insert(record);
				sendSuccessMessage(response,"新增成功");
			}else {
				record.setUpdateBy(getCurrUser());
				wxArticleService.updateDB(record);
